  ...
  return (
    <div className="space-y-4">
      ...
      {messages.map((message) => (
                 <div
           key={message.id}
          className={`flex items-start space-x-3 group mb-4 ${
         ...
         >
          ...
          <div className={`flex-1 max-w-md ${message.senderId === currentUserId ? 'text-right' : ''}`}>
            ...
            <div className="relative">
              {/* Emoji Butonu - Mesaj balonunun dışında */}
              <div className={`absolute top-0 ${message.senderId === currentUserId ? '-left-10' : '-right-10'} opacity-0 group-hover:opacity-100 transition-opacity z-10`}>
                <button
                  onClick={() => setShowEmojiPicker(showEmojiPicker === message.id ? null : message.id)}
                  className="p-1.5 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 rounded-full bg-white dark:bg-gray-800 shadow-md border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all"
                  title="Emoji ekle"
                >
                  😊
                </button>
              ...
              {message.messageType === 'poll' ? (
                <div className="text-sm">
                  ...
                  <div className="text-xs opacity-70">
                      Oy vermek için seçeneklere tıklayın
                  </div>
                </div>
              ...
              {/* Mesaj Aksiyonları - Mesaj balonunun altında */}
              <div className={`mt-1 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity ${message.senderId === currentUserId ? 'justify-end' : 'justify-start'}`}>
                {/* Yanıtla Butonu */}
                {onReplyToMessage && (
                  <button
                    onClick={() => onReplyToMessage(message)}
                    className="p-1 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                    title="Yanıtla"
                  >
                ...
                {onStarMessage && (
                  <button
                    ...
                    className={`p-1 rounded transition-colors ${isMessageStarred && isMessageStarred(message.id) ? 'text-yellow-500 hover:text-yellow-600' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                    title={isMessageStarred && isMessageStarred(message.id) ? 'Önemli işaretlemeyi kaldır' : 'Önemli işaretle'}
                  >
                    <svg className="w-4 h-4" fill={isMessageStarred && isMessageStarred(message.id) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                ...
                {onPinMessage && (
                  <button
                    ...
                    className={`p-1 rounded transition-colors ${isMessagePinned && isMessagePinned(message.id) ? 'text-blue-500 hover:text-blue-600' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                ...
                {deletedMessages && deletedMessages.has(message.id) && onRestoreMessage && (
                  <button
                    ...
                    className="p-1 text-green-500 hover:text-green-600 dark:text-green-400 dark:hover:text-green-300 rounded hover:bg-green-100 dark:hover:bg-green-900 transition-colors"
                    title="Geri yükle"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
              ...
              {messageStatuses && messageStatuses[message.id] && (
                <div className={`mt-1 text-xs ${message.senderId === currentUserId ? 'text-right' : 'text-left'}`}>
                  <span className={`${
