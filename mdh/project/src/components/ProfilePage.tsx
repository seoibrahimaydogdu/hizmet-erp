...
const ProfilePage: React.FC = () => {
  ...
  // Profil değiştiğinde local state'i güncelle
  React.useEffect(() => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Hata durumunda orijinal profili geri yükle
      setProfile(userProfile);
    }
  };
          ...
          <div className="flex gap-2">
            {isEditing && (
              <button
                onClick={() => {
                  setIsEditing(false);
                  // Orijinal profili geri yükle
                  setProfile(userProfile);
                }}
                className="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg text-sm font-medium"
              >
                İptal
              </button>
            )}
            <button
              ...
              disabled={isLoading}
              className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-lg text-sm font-medium disabled:cursor-not-allowed"
            >
              {isEditing ? (
                <>
